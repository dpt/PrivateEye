 ...
 *    Name: create.c
 * Purpose: Hash
 * Version: $Id: create.c,v 1.4 2010-01-13 18:01:18 dpt Exp $
 * ----------------------------------------------------------------------- */
...
/* Alternative Fowler/Noll/Vo FNV-1 hash */
static unsigned int string_hash(const void *a)
{
  const char  *s = a;
  unsigned int h;
...
error hash__create(int size, hash__fn *hash_fn, hash__compare *compare,
                   hash__destroy_key *destroy_key,
                   hash__destroy_value *destroy_value, hash_t **ph)
{
  hash_t *h;
  int     nbins;
  node  **bins;
  ...
  h->hash_fn       = hash_fn ? hash_fn : string_hash;
