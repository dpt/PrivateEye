...
typedef error (*lineparser)(unpickle__state *, char *, void *);
...
static error unpickle__parse_line(unpickle__state *state,
                                  char            *buf,
                                  void            *opaque)
{
  error err;
  ...
  if (keyend == NULL)
    return error_PICKLE_SYNTAX_ERROR; /* end of string */
...
static error unpickle__parse_first_line(unpickle__state *state,
                                        char            *buf,
                                        void            *opaque)
{
  ...
  if (strcmp(buf, signature) != 0)
    return error_PICKLE_INCOMPATIBLE;
                      ...
                      void                          *opaque)
{
  ...
  if (f == 0)
    return error_PICKLE_COULDNT_OPEN_FILE;
