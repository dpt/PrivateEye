...
int main(int argc, char *argv[])
{
    ...
    if (sizeof(unsigned int) != 4) {
        fprintf(stderr, "** Configuration error.  Setting for unsigned int in file md5.h\n");
        ...
        if (*cp == '-') {
            ...
            switch (opt) {
                ...
                case 'D':             /* -Dtext  --  Compute signature of given text */
                    md5_init(&md5c);
                    md5_update(&md5c, (unsigned char *) (cp + 1), strlen(cp + 1));
                    cdata = TRUE;
    ...
    for (; (f > 0) || (i < argc); i++) {
        ...
        if (!cdata) {
            ...
            while ((j = (int) fread(buffer, 1, sizeof buffer, in)) > 0) {
                md5_update(&md5c, buffer, (unsigned) j);
            }
